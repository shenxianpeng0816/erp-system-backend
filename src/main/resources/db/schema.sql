...
CREATE TABLE IF NOT EXISTS `customer` (
    ...
    `country_code`    VARCHAR(2)   DEFAULT NULL COMMENT 'ISO 3166-1 alpha-2 e.g. KE UG TZ',
    `first_transaction_at` DATETIME DEFAULT NULL COMMENT 'first deal time',
    `shop_name`       VARCHAR(200) DEFAULT NULL COMMENT 'customer shop name',
    `invoice_required` TINYINT    NOT NULL DEFAULT 0 COMMENT '1=invoice needed',
    `photo_url`       VARCHAR(500) DEFAULT NULL COMMENT 'customer photo URL path',
...
CREATE TABLE IF NOT EXISTS `sales_order` (
    ...
    `status`              ENUM('DRAFT','PENDING_FIRST_APPROVAL','PENDING_FINAL_APPROVAL','APPROVED','REJECTED','SHIPPED','CONFIRMED','CANCELLED')
                          NOT NULL DEFAULT 'DRAFT',
    `total_amount`        DECIMAL(15,2) NOT NULL DEFAULT 0.00,
    `payment_method`      VARCHAR(50)  DEFAULT NULL COMMENT 'Bank Transfer / Mpesa etc.',
    `price_term`          VARCHAR(50)  DEFAULT NULL COMMENT 'payment term e.g. DDP Kenya',
    `validity_days`       INT          DEFAULT 30,
    `remark`              TEXT         DEFAULT NULL,
    `etr_required`        TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '1 = customer ETR details required',
    `etr_company_name`    VARCHAR(200) DEFAULT NULL COMMENT 'company name for ETR',
    `etr_company_kra_pin` VARCHAR(50) DEFAULT NULL COMMENT 'company KRA PIN for ETR',
...
CREATE TABLE IF NOT EXISTS `receivable` (
    ...
    `received_qty`    INT          NOT NULL DEFAULT 0 COMMENT 'cumulative paid product qty',
...
CREATE TABLE IF NOT EXISTS `payment_record` (
    ...
    `amount`         DECIMAL(15,2) NOT NULL,
    `qty`            INT          DEFAULT NULL COMMENT 'paid product qty',
